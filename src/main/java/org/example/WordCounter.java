...
public class WordCounter {
    ...
    public static Map<String, Integer> calculatedWord() {
        ...
        String[] replaceText=text.toLowerCase().replaceAll("[^a-z\\s]", "").replaceAll("\\s+"," ").split(" ");
        List<String> list=new LinkedList<>();
        list.addAll(Arrays.asList(replaceText));
        for(String item:list){
            words.put(item,Collections.frequency(list,item));
        }
        return words;
    }
}
